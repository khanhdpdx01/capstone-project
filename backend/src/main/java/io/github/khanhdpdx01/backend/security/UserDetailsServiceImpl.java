...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        User user = userRepository.findByUsername(username).orElseThrow(() -> new ApiRequestException("User does not found"));

        List<GrantedAuthority> grandList = new ArrayList<>();

        grandList.add(new SimpleGrantedAuthority(user.getRole()));

        UserPrinciple userPrinciple = new UserPrinciple(user.getUsername(), user.getPassword(), grandList);
        userPrinciple.setUserId(user.getId());
